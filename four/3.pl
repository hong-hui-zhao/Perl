...
OUTER:while($re_or_not){
    ...
    $countdown = 3;
    print "The DNA base is $DNA, please input the complementary base.\nPress R to replay.\n";
    while($countdown>0){
        print "You have $countdown attempts left.\n";
        $input = <STDIN>;
        chomp $input;
        $input = uc $input; # Convert input to uppercase
        if($input eq "E"){
            print "Bye bye!\n";
            exit; # Exit the program
        }
        elsif($input eq $C_DNA){
            print "You are right!\n";
            last;
        }
        elsif($input eq "R"){
